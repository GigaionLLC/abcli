...
- The `gitops/` tree is generated by `seed` and is **gitignored by default** (seeded profiles can carry
  secrets); un-ignore it deliberately once you adopt it as your committed source of truth.
...
## 7. CI/CD posture
Because the private key targets a production tenant, treat automated writes with care:
- **On pull request:** run `validate` + `sync --dry-run` (plan only; exit 3 if changes are pending) and post
  the plan for review — no writes.
- **On merge to main:** run gated `sync --apply` behind a protected environment with required reviewers and a
  serialized concurrency group. `--prune` only via a separate, explicit opt-in; `--limit-writes N` as a
  circuit breaker.
- **Scheduled sync** (optional): a periodic `sync` is what makes bidirectional real — it pulls console edits
  into git as commits and pushes due git changes on a timer, independent of a human git push. Dry-run +
  alert by default; `--apply` only inside an approved window.
- Secrets come from the gitignored `.env`; nothing secret-bearing is committed.
